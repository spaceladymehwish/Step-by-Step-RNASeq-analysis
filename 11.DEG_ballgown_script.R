...
bg_data <- ballgown(dataDir = "quantification", samplePattern = "ERR", pData=pheno_data)
...
hist(results_genes[sig,"logFC"], breaks=50, col="seagreen", xlab="log2(Fold change Disease vs. Normal)", main="Distribution of DEGs Values in ERP vs. TNBC")
...
final_set <- subset(final_without_dots, logFC < -1 & pval < 0.05 | logFC > 1 & pval < 0.05)
